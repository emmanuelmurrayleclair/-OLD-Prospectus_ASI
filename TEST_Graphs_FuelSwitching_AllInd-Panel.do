...
* Define switching as a change in the dominant fuel
gen dominant_fuel = 0
replace dominant_fuel = 1 if s_coal > 0.5
replace dominant_fuel = 2 if s_oil > 0.5
replace dominant_fuel = 3 if s_gas > 0.5
replace dominant_fuel = 4 if s_otherf > 0.5
foreach fuel in 1 2 3 4 {
	gen fuelswitch_to`fuel' = 0
	gen fuelswitch_off`fuel' = 0
	bysort IDnum: replace fuelswitch_to`fuel' = 1 if dominant_fuel == `fuel' & L.dominant_fuel != `fuel'
	bysort IDnum: replace fuelswitch_off`fuel' = 1 if dominant_fuel != `fuel' & L.dominant_fuel == `fuel'
}
foreach i in to off {
	rename fuelswitch_`i'1 fuelswitch_`i'coal
	rename fuelswitch_`i'2 fuelswitch_`i'oil
	rename fuelswitch_`i'3 fuelswitch_`i'gas
	rename fuelswitch_`i'4 fuelswitch_`i'otherf
}
replace fuelswitch_to = 1 if fuelswitch_tocoal == 1 | fuelswitch_tooil == 1 | fuelswitch_togas == 1 | fuelswitch_tootherf == 1
replace fuelswitch_off = 1 if fuelswitch_offcoal == 1 | fuelswitch_offoil == 1 | fuelswitch_offgas == 1 | fuelswitch_offotherf == 1
